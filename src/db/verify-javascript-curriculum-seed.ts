/**
 * JavaScript curriculum seed 前後只讀 fingerprint：
 *   npx tsx --env-file=.env src/db/verify-javascript-curriculum-seed.ts
 */
...
async function main() {
  const protectedFingerprints = rowsOf(await db.execute(sql`
    SELECT
      'react_unit' AS entity,
      count(*)::int AS "rowCount",
      md5(coalesce(string_agg(u.id || ':' || u.title || ':' || u.position, ',' ORDER BY u.id), '')) AS digest
    FROM unit u
