/**
 * Python curriculum seed 前後只讀 fingerprint：
 *   npx tsx --env-file=.env src/db/verify-python-curriculum-seed.ts
 */
...
async function main() {
  const protectedFingerprints = rowsOf(await db.execute(sql`
    SELECT
      'existing_path' AS entity,
      count(*)::int AS "rowCount",
      md5(coalesce(string_agg(p.id || ':' || p.title || ':' || p.description || ':' || p.subject || ':' || p.code_language || ':' || p.status || ':' || p.position, ',' ORDER BY p.id), '')) AS digest
    FROM learning_path p
    WHERE p.id IN ('javascript-interview-core', 'typescript-frontend-core', 'react-junior-mid')
    UNION ALL
    SELECT
      'existing_recommendation',
      count(*)::int,
      md5(coalesce(string_agg(r.path_id || ':' || r.recommended_path_id, ',' ORDER BY r.path_id, r.recommended_path_id), ''))
    FROM learning_path_recommendation r
    WHERE r.path_id IN ('javascript-interview-core', 'typescript-frontend-core', 'react-junior-mid')
       OR r.recommended_path_id IN ('javascript-interview-core', 'typescript-frontend-core', 'react-junior-mid')
    UNION ALL
    SELECT
      'existing_unit',
      count(*)::int,
      md5(coalesce(string_agg(u.id || ':' || u.path_id || ':' || u.title || ':' || u.position, ',' ORDER BY u.id), ''))
    FROM unit u
    WHERE u.path_id IN ('javascript-interview-core', 'typescript-frontend-core', 'react-junior-mid')
    UNION ALL
    SELECT
      'existing_lesson',
      count(*)::int,
    ...
    FROM lesson_session
    UNION ALL
    SELECT 'practice_session', count(*)::int,
    ...
    UNION ALL
    SELECT 'review_session', count(*)::int,
