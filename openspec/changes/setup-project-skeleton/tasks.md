...
- [ ] 2.2 安裝 Drizzle ORM + drizzle-kit，設定 `drizzle.config.ts` 與 migration scripts
- [ ] 2.3 跑一次空 migration 驗證 generate → migrate 流程可用

## 3. 認證

- [ ] 3.1 安裝 Better Auth（email + password）+ Drizzle adapter，產生 auth 相關 table migration
- [ ] 3.2 註冊頁與登入頁（含錯誤訊息：email 已存在、憑證錯誤）
- [ ] 3.3 登出功能與未登入導向登入頁的 middleware 保護
- [ ] 3.4 seed script 建立固定 demo 帳號，登入頁加「Demo 登入」按鈕

## 4. 首頁與部署

- [ ] 4.1 登入後空白首頁：顯示登入狀態 + 登出入口
